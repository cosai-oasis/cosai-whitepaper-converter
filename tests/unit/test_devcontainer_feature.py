...
def test_install_script_overwrites_bundled_puppeteer_config():
    """
    Test that install.sh copies the freshly generated puppeteerConfig.json
    after copying bundled converter assets.

    Given: install.sh bundles converter assets including puppeteerConfig.json
    When: configure-chromium.sh has generated a platform-correct config
    Then: Install script overwrites bundled config with the generated one
    """
    with open(INSTALL_SCRIPT) as f:
        content = f.read()

    # Must reference SCRIPT_DIR/assets/puppeteerConfig.json (generated by configure-chromium.sh)
    # to overwrite the stale bundled one from CONVERTER_DIR/assets/
    assert "assets/puppeteerConfig.json" in content, \
        "Install script must reference generated puppeteerConfig.json"

    # The overwrite must come AFTER the bundled asset copy loop
    # Find the asset copy loop and the puppeteer overwrite
    asset_loop_pos = content.find("for asset in")
    puppeteer_overwrite = content.find("SCRIPT_DIR")
    # Both must exist, and SCRIPT_DIR reference for puppeteer should come after asset loop
    assert asset_loop_pos > 0, "Install script must have asset copy loop"

    # Look for the specific pattern: copying from SCRIPT_DIR/assets/ (generated config)
    # vs CONVERTER_DIR/assets/ (bundled config)
    assert "SCRIPT_DIR" in content and "puppeteerConfig" in content, \
        "Install script must copy generated puppeteerConfig.json from SCRIPT_DIR/assets/"


def test_puppeteer_config_uses_auto_detection():
    """
    Test that the committed puppeteerConfig.json uses auto-detection mode.

    Given: assets/puppeteerConfig.json is bundled into the feature
    When: Feature is installed on any platform
    Then: Config should NOT have hardcoded executablePath (configure-chromium.sh sets it at install time)
    """
    repo_root = FEATURE_DIR.parent.parent
    config_path = repo_root / "assets" / "puppeteerConfig.json"
    with open(config_path) as f:
        config = json.load(f)

    assert "executablePath" not in config, \
        "puppeteerConfig.json should NOT have hardcoded executablePath; " \
        "configure-chromium.sh generates it at install time"
